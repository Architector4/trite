...
/// An interface object for altering the world state based on a continuum.
///
/// For an example, see the [crate] documentation page.
pub struct WorldContinuumInterface<'a, C: Continuum> {
    world: &'a mut World,
    continuum: core::marker::PhantomData<C>,
}

impl<'a, C: Continuum> WorldContinuumInterface<'a, C> {
    /// Create a new instance. For ergonomics, you might want to use [`WorldTimeTravel::continuum`]
    /// instead.
    ///
    /// [`WorldTimeTravel::continuum`]: super::world_methods::WorldTimeTravel::continuum
    ...
    pub fn rewind_to(&mut self, to: Duration) -> Result<Duration, OutOfRecordedRangeError> {
        self.rewind_to_with_policies(
            to,
    ...
    /// If the specified time is outside of range recorded for this continuum, this does nothing
    /// and returns an [`OutOfRecordedRangeError`].
    pub fn rewind_to_with_policies(
        &mut self,
        to: Duration,
    ...
    /// If the specified time is outside of range recorded for this continuum, this does nothing
    /// and returns an [`OutOfRecordedRangeError`].
    pub fn interpolate_to(&mut self, to: Duration) -> Result<Duration, OutOfRecordedRangeError> {
        self.interpolate_to_with_policy(to, OutOfTimelineRangePolicy::default())
    }
    ...
    /// If the specified time is outside of range recorded for this continuum, this does nothing
    /// and returns an [`OutOfRecordedRangeError`].
    pub fn interpolate_to_with_policy(
        &mut self,
        to: Duration,
        out_of_timeline_range_policy: OutOfTimelineRangePolicy,
    ) -> Result<Duration, OutOfRecordedRangeError> {
        ...
        self.world
            .run_schedule(TimeTravelSchedules::Interpolating(continuum));
        self.world.remove_resource::<InterpolateTo<C>>();
    ...
    pub fn rotate_buffers(&mut self, delete_before: Duration, current_time: Duration) {
        ...
        if let Some(mut cont_time) = self.world.get_resource_mut::<ContinuumTime<C>>() {
            ...
            Timekeep::<C>::register_into_world(self.world);
            self.world
                .insert_resource(Timekeep::with_continuum(continuum.clone()));
            self.world.insert_resource(ContinuumTime {
                time: current_time,
                continuum: continuum.clone(),
            });
        }

        self.world.insert_resource(RotateBuffers {
            delete_before,
            current_time,
            continuum: continuum.clone(),
        });
        self.world
            .run_schedule(TimeTravelSchedules::RotatingBuffers(continuum));
        self.world.remove_resource::<RotateBuffers<C>>();
    }
    ...
    pub fn insert_into_buffers(&mut self, current_time: Duration) {
        self.rotate_buffers(Duration::ZERO, current_time);
    }
    ...
    pub fn delete_after(&mut self, delete_after: Duration) {
        ...
        self.world.insert_resource(DeleteAfter {
            delete_after,
            continuum: continuum.clone(),
        });
        self.world
            .run_schedule(TimeTravelSchedules::DeletingAfter(continuum));
        self.world.remove_resource::<DeleteAfter<C>>();
    }
    ...
    pub fn clear_timelines(&mut self) {
        ...
        self.world
            .run_schedule(TimeTravelSchedules::Clearing(continuum));
    }
    ...
    /// # Panics
    ///
    /// Panics if something in the schedule [`TimeTravelSchedules::AccountingForChanges`] deletes
    ...
    pub fn account_for_changes(&mut self, overwrite_states: usize) -> bool {
        ...
        self.world
            .run_schedule(TimeTravelSchedules::AccountingForChanges(continuum));
        self.world
            .remove_resource::<AccountForChanges<C>>()
    ...
    pub fn clean_up_empty(&mut self) {
        let continuum = C::default();

        #[cfg(feature = "logging")]
        trace!("{continuum:?}: Clearing empty components and resources...");

        self.world.insert_resource(CleanUpEmpty {
            continuum: continuum.clone(),
        });
        self.world
            .run_schedule(TimeTravelSchedules::CleaningUpEmptyDetecting(
                continuum.clone(),
            ));
        self.world
            .run_schedule(TimeTravelSchedules::CleaningUpEmptyPerforming(continuum));
        self.world.remove_resource::<CleanUpEmpty<C>>();
    }
    ...
    pub fn clean_up_disabled(&mut self) {
        let continuum = C::default();
        ...
        self.world
            .run_schedule(TimeTravelSchedules::CleaningUpDisabledEntities(continuum));
    }
}
...
mod tests {
    ...
    use super::super::world_methods::WorldTimeTravel;
    ...
    fn adding_works() {
        ...
        world.continuum::<TestC>().insert_into_buffers(secs(1));
        world.continuum::<TestC>().insert_into_buffers(secs(2));
    ...
    fn time_out_of_range() {
        ...
        assert_eq!(world.continuum::<TestC>().rewind_to(secs(1)), Ok(secs(1)));
        assert_eq!(world.continuum::<TestC>().rewind_to(secs(2)), Ok(secs(2)));

        // Rewinding to outside
        assert_eq!(
            world.continuum::<TestC>().rewind_to(secs(0)),
            Err(OutOfRecordedRangeError)
        );
        assert_eq!(
            world.continuum::<TestC>().rewind_to(secs(5)),
            Err(OutOfRecordedRangeError)
        );
